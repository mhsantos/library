 ...
 */
public abstract class DurabilityCoordinator implements Recoverable,
        BatchExecutable {
    ...
    public DurabilityCoordinator() {
        try {
            ...
            java.util.logging.Logger.getLogger(
                    DurabilityCoordinator.class.getName()).log(Level.SEVERE,
                    null, ex);
        }
    }
    ...
    private byte[][] executeBatch(byte[][] commands, MessageContext[] msgCtx,
            boolean noop) {
        ...
        // During the consensus IDs contained in this batch of commands none of the replicas is
        // supposed to take a checkpoint, so the replica will only execute the command and
        // return the replies
        if (checkpointIndex == -1) {
            ...
            Logger.println("(DurabilityCoordinator.executeBatch) Storing message batch in the state log for consensus "
                    + cid);
            boolean writeToBeforeCkpLog = msgCtx[msgCtx.length - 1].getConsensusId() < globalCheckpointPeriod;
            saveCommands(commands, msgCtx, true, writeToBeforeCkpLog);
        } else {
            // there is a replica supposed to take the checkpoint. In this case,
            // the commands
            // have to be executed in two steps. First the batch of commands
            // containing commands
            // until the checkpoint period is executed and the log saved or
            // checkpoint taken
            // if this replica is the one supposed to take the checkpoint. After
            // the checkpoint
            // or log, the pointer in the log is updated and then the remaining
            // portion of the
            ...
            if (secondHalf.length > 0) {
                System.arraycopy(commands, checkpointIndex + 1, secondHalf, 0,
                        commands.length - (checkpointIndex + 1));
                System.arraycopy(msgCtx, checkpointIndex + 1, secondHalfMsgCtx,
                        0, commands.length - (checkpointIndex + 1));
            } else {
            ...
            cid = firstHalfMsgCtx[checkpointIndex].getConsensusId();
            if (cid % globalCheckpointPeriod == replicaCkpIndex && lastCkpCID < cid) {
                Logger.println("(DurabilityCoordinator.executeBatch) Performing checkpoint for consensus "
                        + cid);
                stateLock.lock();
                ...
                lastCkpCID = cid;
                if (cid < globalCheckpointPeriod) {
                    saveCommands(firstHalf, firstHalfMsgCtx, false, true);
                }
            } else {
                Logger.println("(DurabilityCoordinator.executeBatch) Storing message batch in the state log for consensus "
                        + cid);
                boolean writeToBeforeCkpLog = firstHalfMsgCtx[firstHalfMsgCtx.length - 1].getConsensusId() < globalCheckpointPeriod;
                saveCommands(commands, msgCtx, true, writeToBeforeCkpLog);
            }
            ...
            if (secondHalf.length > 0) {
                ...
                Logger.println("(DurabilityCoordinator.executeBatch) Storing message batch in the state log for consensus "
                        + cid);
                boolean writeToBeforeCkpLog = secondHalfMsgCtx[secondHalfMsgCtx.length - 1].getConsensusId() < globalCheckpointPeriod;
                saveCommands(secondHalf, secondHalfMsgCtx, true, writeToBeforeCkpLog);
                System.arraycopy(secondHalfReplies, 0, replies,
                        firstHalfReplies.length, secondHalfReplies.length);
            }
        }
    ...
    /**
     * Iterates over the commands to find if any replica took a checkpoint. When
     * a replica take a checkpoint, it is necessary to save in an auxiliary
     * table the position in the log in which that replica took the checkpoint.
     * It is used during state transfer to find lower or upper log portions to
     * be restored in the recovering replica. This iteration over commands is
     * needed due to the batch execution strategy introduced with the durable
     * techniques to improve state management. As several consensus instances
     * can be executed in the same batch of commands, it is necessary to
     * identify if the batch contains checkpoint indexes.
     *
     * @return the index in which a replica is supposed to take a checkpoint. If
     *         there is no replica taking a checkpoint during the period
     *         comprised by this command batch, it returns -1
     */
     ...
     * 
     * @param msgCtx
     *            the message context of the commands executed by the replica.
    ...
    public int setState(ApplicationState recvState) {
        ...
        if (recvState instanceof CSTState) {
            ...
            bftsmart.tom.util.Logger
                    .println("(DurabilityCoordinator.setState) I'm going to update myself from CID "
            ...
            System.out.print("--- Installing log from "
                    + (lastCheckpointCID + 1) + " to " + lastCID);

            for (int cid = lastCheckpointCID + 1; cid <= lastCID; cid++) {
                try {
                    bftsmart.tom.util.Logger
                            .println("(DurabilityCoordinator.setState) interpreting and verifying batched requests for CID "
                                    + cid);
                    CommandsInfo cmdInfo = state.getMessageBatch(cid);
                    byte[][] commands = cmdInfo.commands;
                    MessageContext[] msgCtx = cmdInfo.msgCtx;

                    if (commands == null || msgCtx == null
                            || msgCtx[0].isNoOp()) {
                        continue;
                    }
    ...
    private void saveState(byte[] snapshot, int lastCID) {
        ...
        logLock.unlock();
        Logger.println("(TOMLayer.saveState) Finished saving state of CID "
                + lastCID);
    }
     ...
     * @param msgCtx
     */
    private void saveCommands(byte[][] commands, MessageContext[] msgCtx, boolean writeToRegularLog, boolean writeToBeforeCkpLog) {
        if (!config.isToLog())
            return;
        ...
        for (int i = 0; i <= msgCtx.length; i++) {
            if (i == msgCtx.length) { 
                // the batch command contains only one command or it is the last position of the array
                byte[][] batch = Arrays.copyOfRange(commands, batchStart, i);
                MessageContext[] batchMsgCtx = Arrays.copyOfRange(msgCtx, batchStart, i);
                if (writeToRegularLog) {
                    log.addMessageBatch(batch, batchMsgCtx, cid);
                    log.setLastCID(cid, globalCheckpointPeriod, checkpointPortion);
                }
                if (writeToBeforeCkpLog) {
                    log.addMessageBatchBeforeGlobal(batch, batchMsgCtx, cid);
                }
                // if(batchStart > 0)
                // System.out.println("Last batch: " + commands.length + "," +
                // batchStart + "-" + i + "," + batch.length);
            } else {
                if (msgCtx[i].getConsensusId() > cid) {
                	// saves commands when the CID changes or when it is the last batch
                    byte[][] batch = Arrays.copyOfRange(commands, batchStart, i);
                    MessageContext[] batchMsgCtx = Arrays.copyOfRange(msgCtx, batchStart, i);
                    // System.out.println("THERE IS MORE THAN ONE CID in this batch."
                    // + commands.length + "," + batchStart + "-" + i + "," +
                    // batch.length);
                    if (writeToRegularLog) {
                        log.addMessageBatch(batch, batchMsgCtx, cid);
                        log.setLastCID(cid, globalCheckpointPeriod, checkpointPortion);
     ...
     * and logging the only information important in MessageContext is the
    ...
    public byte[] getCurrentStateHash() {
        ...
        System.out.println("--- State size: " + currentState.length
                + " Current state Hash: " + Arrays.toString(currentStateHash));
    ...
    public void Op(int CID, byte[] requests, MessageContext msgCtx) {
        // Requests are logged within 'executeBatch(...)' instead of in this
        // method.
    }
